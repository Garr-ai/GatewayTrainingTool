  ...
  return (
    <div className="flex flex-col gap-4">
      <header>
        ...
        <p className="mt-0.5 text-sm text-slate-400">Sessions across all assigned classes</p>
      </header>
        ...
        <>
          <div className="bg-gw-surface rounded-[10px] overflow-x-auto">
            <table className="min-w-full text-xs">
              ...
              <tbody>
                {slots.map(s => (
                  <tr key={s.id} className="border-b border-white/[0.03] hover:bg-gw-elevated transition-colors">
                    <td className="px-3 py-2 text-slate-200 font-medium">{s.slot_date}</td>
                    <td className="px-3 py-2 text-slate-300">{s.start_time}–{s.end_time}</td>
                    <td className="px-3 py-2 text-slate-400">{s.classes.name}</td>
                    <td className="px-3 py-2 text-slate-400 hidden sm:table-cell">{s.group_label ?? '—'}</td>
                    <td className="px-3 py-2 text-slate-500 hidden md:table-cell">{s.notes ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
