  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <header className="flex-shrink-0">
        <h2 className="text-lg font-semibold text-slate-900">Student Payroll</h2>
        <p className="mt-0.5 text-xs text-slate-500">
          Aggregated training hours per student across all classes
        </p>
      </header>
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center">
            {hasActiveFilters ? (
              ...
              <>
                <p className="text-sm text-slate-600">No student hours logged yet.</p>
          ...
          <>
            <PayrollTable rows={rows} personLabel="Student" />
            <Pagination page={page} limit={limit} total={total} onPageChange={setPage} itemLabel="student" />
          </>
        )}
      </div>
    </div>
