  ...
  return (
    <section className="bg-gw-surface rounded-[10px] p-4 flex flex-col gap-4">
        ...
        {(['my-hours', 'student-hours'] as const).map(t => (
          <button
            ...
            className={`px-3 py-2 text-xs font-medium whitespace-nowrap border-b-2 transition-colors ${
              activeTab === t ? 'border-gw-blue text-gw-blue' : 'border-transparent text-slate-500 hover:text-slate-300'
            }`}
          >
      ...
      {activeTab === 'my-hours' && (
        <>
          <div className="flex items-center justify-between gap-2">
            <div>
              <p className="text-xs font-semibold text-slate-300">Total: <span className="text-gw-blue">{totalMyHours}h</span></p>
          ...
          {myFormOpen && (
            <form onSubmit={handleMySave} className="rounded-[10px] border border-white/[0.06] bg-gw-elevated p-3">
              <p className="text-xs font-medium text-slate-400 mb-2">{myEditing ? 'Edit hours entry' : 'Log my hours'}</p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Hours
                ...
                <div className="col-span-2 sm:col-span-4 flex justify-end gap-2">
                  <button type="button" onClick={resetMyForm} className="rounded-md bg-gw-surface text-slate-200 border border-white/10 px-3 py-1.5 text-xs font-semibold">Cancel</button>
          ...
          {loading ? (
            ...
            <div className="bg-gw-elevated rounded-[10px] overflow-x-auto">
              <table className="min-w-full text-xs">
                ...
                <tbody>
                  {trainerHours.map(h => (
                    <tr key={h.id} className="border-b border-white/[0.03] hover:bg-gw-surface transition-colors">
                      <td className="px-3 py-2 text-slate-200">{h.log_date}</td>
                      <td className="px-3 py-2 text-slate-200 font-medium">{h.hours}h</td>
      ...
      {activeTab === 'student-hours' && (
        <>
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs text-slate-400">Log hours for enrolled students.</p>
            {!archived && (
              <div className="flex gap-2">
                <button type="button" onClick={() => { setStuFormOpen(!stuFormOpen || stuBulkMode); setStuBulkMode(false); if (stuEditing) resetStuForm() }} className="rounded-md bg-gw-surface border border-white/10 text-slate-200 font-semibold px-3 py-1.5 text-xs hover:bg-gw-elevated transition-all">
          ...
          {stuFormOpen && !stuBulkMode && (
            <form onSubmit={handleStuSave} className="rounded-[10px] border border-white/[0.06] bg-gw-elevated p-3">
              <p className="text-xs font-medium text-slate-400 mb-2">{stuEditing ? 'Edit student hours' : 'Log student hours'}</p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Date
                ...
                <div className="col-span-2">
                  <label className="block text-xs font-medium text-slate-400 mb-1">Student
                ...
                <div className="col-span-2 sm:col-span-4 flex justify-end gap-2">
                  <button type="button" onClick={resetStuForm} className="rounded-md bg-gw-surface text-slate-200 border border-white/10 px-3 py-1.5 text-xs font-semibold">Cancel</button>
          ...
          {/* Bulk form */}
          {stuFormOpen && stuBulkMode && (
            <form onSubmit={handleBulkSave} className="rounded-[10px] border border-white/[0.06] bg-gw-elevated p-3">
              <p className="text-xs font-medium text-slate-400 mb-3">Bulk log hours for all students</p>
              ...
              <div className="bg-gw-surface rounded-[10px] overflow-hidden mb-3">
                <table className="min-w-full text-xs">
                  <thead>
                    <tr className="bg-white/[0.02] border-b border-white/[0.06]">
                      ...
                      <tr key={e.id} className="border-b border-white/[0.03]">
                        ...
                        <td className="px-2 py-1">
                          <input
                            ...
                            className="w-20 bg-gw-elevated border border-white/10 rounded px-1.5 py-1 text-[10px] text-slate-200 outline-none"
                            placeholder="0"
                          />
                        </td>
                      </tr>
              ...
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => { setStuFormOpen(false); setStuBulkMode(false) }} className="rounded-md bg-gw-surface text-slate-200 border border-white/10 px-3 py-1.5 text-xs font-semibold">Cancel</button>
                ...
                <tbody>
                  {studentHours.map(h => {
                    ...
                    return (
                      <tr key={h.id} className="border-b border-white/[0.03] hover:bg-gw-surface transition-colors">
                        <td className="px-3 py-2 text-slate-200">{h.log_date}</td>
                        <td className="px-3 py-2 text-slate-300">{enr?.student_name ?? '—'}</td>
                        <td className="px-3 py-2 text-slate-200 font-medium">{h.hours}h</td>
