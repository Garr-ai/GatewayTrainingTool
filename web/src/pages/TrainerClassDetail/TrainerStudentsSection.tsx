      ...
      {loading ? (
        ...
        <div className="bg-gw-elevated rounded-[10px] overflow-x-auto">
          <table className="min-w-full text-xs">
            ...
            <tbody>
              {enrollments.map(e => (
                <tr key={e.id} className="border-b border-white/[0.03] hover:bg-gw-surface transition-colors duration-100">
                  <td className="px-3 py-2 text-slate-200">{e.student_name}</td>
                  <td className="px-3 py-2 text-slate-400 hidden sm:table-cell">{e.student_email}</td>
                  ...
                  <td className="px-3 py-2 text-slate-400 hidden sm:table-cell">{e.group_label ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
